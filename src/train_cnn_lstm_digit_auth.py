'''
python train_cnn_lstm_digit_auth.py --data_dir dataset --seq_len 256 --epochs 80
'''
import time
import argparse
import random
...
def build_records(data_dir: str):
    ...
    if len(user_dirs) < 2:
        raise ValueError("At least 2 users are required for user authentication.")
...
def split_records(records, train_ratio=0.6, val_ratio=0.2, seed=42):
    """
    闭集用户认证划分：
    每个 user/digit 内划分 train/val/test。
    train 用作 enrollment set；
    val 用于选择认证 threshold；
    test 用于最终计算 FAR/FRR。

    如果后续数据中有 session 信息，建议改为 session-wise split，
    避免同一采集 session 的相似样本同时进入训练集和测试集。
    """
    ...
    if len(val_records) == 0 or len(test_records) == 0:
        raise ValueError(
            "Validation or test set is empty. "
            "Please collect more samples for each user/digit."
        )

    return train_records, val_records, test_records


def temporal_binning(t: np.ndarray, p: np.ndarray, seq_len: int):
    """
    将变长压力信号转换为固定长度序列。
    这里不用 spline interpolation，而是 temporal binning：
    沿归一化时间轴分成 seq_len 个 bin，每个 bin 取原始压力点均值。
    """
    if len(p) == 0:
...
def preprocess_csv(csv_path: str, seq_len: int):
    ...
    # 4. 固定长度 shape-normalized sequence
...
class CNNLSTMAuthModel(nn.Module):
    """
    CNN-LSTM 多任务模型：
    CNN 提取局部压力波形特征；
    LSTM 建模长程时序依赖；
    ...
    def __init__(
        ...
        self.embedding = nn.Sequential(
            nn.Linear(lstm_hidden * 2, emb_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
        )
    ...
    def forward(self, x):
        ...
        x: [batch, 1, seq_len]
        """
        h = self.cnn(x)              # [batch, channels, length]
        h = h.transpose(1, 2)        # [batch, length, channels]

        lstm_out, _ = self.lstm(h)   # [batch, length, 2 * hidden]

        # 使用最后一个时间步作为序列表征
        h_last = lstm_out[:, -1, :]  # [batch, 2 * hidden]

        emb = self.embedding(h_last)
...
def train_one_epoch(model, loader, optimizer, device, user_loss_weight=1.0):
    model.train()
...
def evaluate_classification(model, loader, device):
    ...
    for x, digit, user in loader:
        ...
        digit_correct += int((pred_digit == digit).sum().item())
        user_correct += int((pred_user == user).sum().item())
        total += int(x.size(0))

    return {
        "digit_acc": digit_correct / max(total, 1),
        "user_identification_acc": user_correct / max(total, 1),
    }
...
def build_user_centroids(train_emb, train_users, num_users):
    """
    每个用户用训练集 embedding 的均值作为 enrollment template。
    """
    centroids = []
    ...
    for u in range(num_users):
        ...
        if len(idx) == 0:
            raise ValueError(f"No enrollment samples found for user {u}")
...
def make_auth_scores(
    emb,
    users,
    centroids,
    max_impostors_per_sample=None,
    seed=42,
):
    """
    genuine score:
        测试样本与真实用户 template 的相似度。

    impostor score:
        测试样本冒充其他用户时，与其他用户 template 的相似度。

    相似度使用 cosine similarity。
    """
    rng = np.random.default_rng(seed)

    score_matrix = emb @ centroids.T
    ...
    for i, true_user in enumerate(users):
        ...
        if (
            max_impostors_per_sample is not None
            and len(impostor_users) > max_impostors_per_sample
        ):
...
def choose_threshold_by_val_eer(genuine_scores, impostor_scores):
    ...
    val_far = float(fars[idx])
    val_frr = float(frrs[idx])
    val_eer = float((val_far + val_frr) / 2.0)

    return threshold, val_far, val_frr, val_eer
...
def compute_far_frr_curve(
    ...
    """
    计算不同 authentication threshold 下的 FAR 和 FRR 曲线。

    FAR:
        impostor score >= threshold 的比例。

    FRR/FFR:
        genuine score < threshold 的比例。
    """
    ...
    eer_far = float(fars[eer_idx])
    eer_frr = float(frrs[eer_idx])
...
def plot_far_frr_threshold_curve(
    thresholds,
    fars,
    frrs,
    ...
    eer_idx = int(np.argmin(np.abs(fars - frrs)))
    eer_threshold = float(thresholds[eer_idx])
    eer_value = float((fars[eer_idx] + frrs[eer_idx]) / 2.0)

    plt.scatter(
        [eer_threshold],
        [eer_value],
        marker="o",
        label=f"EER = {eer_value:.3f} at threshold {eer_threshold:.3f}",
    )

    if selected_threshold is not None:
        selected_idx = int(np.argmin(np.abs(thresholds - selected_threshold)))
        selected_far = float(fars[selected_idx])
        selected_frr = float(frrs[selected_idx])
        ...
        plt.scatter([selected_threshold], [selected_far], marker="x")
        plt.scatter([selected_threshold], [selected_frr], marker="x")
    ...
    plt.savefig('../result/lstm/far-ffr.pdf', format='pdf', bbox_inches='tight', pad_inches=0.26)
...
def plot_training_curves(history):
    ...
    plt.plot(epochs, history["val_user_acc"], label="User Validation Accuracy", linewidth=linewidth)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('../result/lstm/Train Accuracy.pdf', format='pdf', bbox_inches='tight', pad_inches=0.26)

    # plt.figure(figsize=(7, 5))
    plt.figure(figsize=(fig_width, fig_height))
    plt.rcParams["font.family"] = "Arial"
    plt.plot(epochs, history["train_loss"], label="Train Loss", linewidth=linewidth)
    # 获取当前坐标轴
    ax = plt.gca()
    # 设置边框宽度
    ax.spines['top'].set_linewidth(5)  # 设置顶部边框宽度
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('../result/lstm/Recognition Loss.pdf', format='pdf', bbox_inches='tight', pad_inches=0.26)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="默认使用所有 impostor users。数据很大时可设为 20 或 50。",
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        score = (
            val_metrics["digit_acc"]
            + val_metrics["user_identification_acc"]
        )
        ...
        history["train_digit_acc"].append(train_metrics["digit_acc"])
        history["train_user_acc"].append(train_metrics["user_identification_acc"])
        history["val_digit_acc"].append(val_metrics["digit_acc"])
        history["val_user_acc"].append(val_metrics["user_identification_acc"])

        print(
            f"Epoch {epoch:03d}/{args.epochs} | "
            f"loss={train_loss:.4f} | "
            f"train_digit_acc={train_metrics['digit_acc']:.4f} | "
            f"val_digit_acc={val_metrics['digit_acc']:.4f} | "
            f"train_user_acc={train_metrics['user_identification_acc']:.4f} | "
            f"val_user_acc={val_metrics['user_identification_acc']:.4f}"
        )
    ...
    # 提取 embedding
    train_emb, train_users, _ = extract_embeddings(
        model,
        train_loader,
        device,
    )

    val_emb, val_users, _ = extract_embeddings(
        model,
        val_loader,
        device,
    )

    test_emb, test_users, _ = extract_embeddings(
        model,
        test_loader,
        device,
    )
    ...
    centroids = build_user_centroids(
        train_emb,
        train_users,
        num_users,
    )

    # validation set 选择认证 threshold
    ...
    threshold, val_far, val_frr, val_eer = choose_threshold_by_val_eer(
        val_genuine,
        val_impostor,
    )

    # 计算并绘制 validation set 的 FAR/FRR-threshold 曲线
    val_thresholds, val_fars, val_frrs, val_eer_threshold, val_eer_curve = compute_far_frr_curve(
        val_genuine,
        val_impostor,
        num_thresholds=2001,
    )

    plot_far_frr_threshold_curve(
        val_thresholds,
        val_fars,
        val_frrs,
        selected_threshold=threshold,
        output_path="cnn_lstm_val_far_frr_threshold_curve.pdf",
        title="CNN-LSTM Validation FAR/FRR vs Authentication Threshold",
    )
    ...
    save_far_frr_curve_csv(
        ...
        val_fars,
        val_frrs,
        output_path="cnn_lstm_val_far_frr_threshold_curve.csv",
    )
    ...
    print(f"Validation threshold by EER    : {threshold:.4f}")
    ...
    hist_df["digit_generalization_gap"] = (
        hist_df["train_digit_acc"] - hist_df["val_digit_acc"]
    )
    hist_df["user_generalization_gap"] = (
        hist_df["train_user_acc"] - hist_df["val_user_acc"]
    )
    hist_df.to_csv("cnn_lstm_training_history.csv", index=False)
    ...
    torch.save(
        ...
        "best_cnn_lstm_digit_auth.pt",
    )

    print("\nSaved training history to cnn_lstm_training_history.csv")
    print("Saved model to best_cnn_lstm_digit_auth.pt")
